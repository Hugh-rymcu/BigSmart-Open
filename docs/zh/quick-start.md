...
1. 插入 MicroSD 卡，建议先准备好 `/music`、`/videos`、`/background` 等目录。
...
固件启动时会自动创建 `/sdcard/videos` 和 `/sdcard/background`。`music` 目录建议手动创建。
